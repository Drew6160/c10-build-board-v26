...
function generateWireVizYAML(loom) {
  ...
  // ── Color normalizer: converts "GY/BK" → "GYBK" for WireViz 0.4.x ──
  function normalizeColor(color) {
    if (!color) return "WH";
    return color.replace("/", "");          // "GY/BK" → "GYBK", "RD" → "RD"
  }
  ...
  nodeIds.forEach(id => {
    ...
    yaml += `  ${label}:\n`;
    yaml += `    type: ${node.partNumber || "unknown"}\n`;
    yaml += `    notes: "${(node.notes || "").replace(/"/g, "'").split("\n")[0]}"\n`;

    // Pin labels from connectors array — fall back to a safe default pincount
    if (node.connectors && node.connectors.length > 0) {
      const conn = node.connectors[0];
      const pinCount = conn.pinCount || (conn.pins && conn.pins.length) || 2;
      yaml += `    pincount: ${pinCount}\n`;
      if (conn.pins && conn.pins.length > 0) {
        yaml += `    pinlabels: [${conn.pins.map(p => p.label).join(", ")}]\n`;
      }
    } else {
      // No connector data — emit a safe 2-pin stub so WireViz doesn't crash
      yaml += `    pincount: 2\n`;
    }
    yaml += `\n`;
  });
  ...
  filteredEdges.forEach((edge, i) => {
    ...
    // Normalize color — concatenate base + stripe, strip any slash
    const baseColor   = normalizeColor(edge.color || "WH");
    const stripeColor = edge.colorStripe ? normalizeColor(edge.colorStripe) : null;
    const colorStr    = stripeColor ? `${baseColor}${stripeColor}` : baseColor;

    // wirecount: 1 for single conductor; use actual count if explicitly set
    const wirecount = edge.wirecount || 1;

    yaml += `  ${cableId}:\n`;
    yaml += `    gauge: ${edge.wireOverride || "18 AWG"}\n`;
    yaml += `    colors: [${colorStr}]\n`;
    yaml += `    wirecount: ${wirecount}\n`;
    if (edge.shield) {
      yaml += `    shield: true\n`;
      yaml += `    shieldcolor: GN\n`;
    }
  ...
  filteredEdges.forEach((edge, i) => {
    const cableId = `W${String(i+1).padStart(2,"0")}_${edge.from}_${edge.to}`;
    const fromNode = (STATE.nodes[edge.from]?.label || edge.from).replace(/[^a-zA-Z0-9_]/g,"_");
    const toNode   = (STATE.nodes[edge.to]?.label   || edge.to  ).replace(/[^a-zA-Z0-9_]/g,"_");
