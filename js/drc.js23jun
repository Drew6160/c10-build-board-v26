...
function generateWireVizYAML(loom) {
  ...
  nodeIds.forEach(id => {
    ...
    // Pin labels from connectors array
  ...
  filteredEdges.forEach((edge, i) => {
    ...
    yaml += `    wirecount: 1\n`;
    // WireViz 0.4.1 Cable schema: shield takes a single color value, not a
    // boolean + separate shieldcolor pair. twisted is not a valid Cable
    // field in this version — fold it into notes instead of emitting a key
    // WireViz will reject outright (causes a hard crash, zero output files).
    if (edge.shield) {
      yaml += `    shield: GN\n`;
    }
    if (edge.length)  yaml += `    length: ${edge.length} in\n`;
    const twistedNote = edge.twisted ? "Twisted pair. " : "";
    const baseNotes = edge.notes ? edge.notes.replace(/"/g,"'") : "";
    if (twistedNote || baseNotes) {
      yaml += `    notes: "${twistedNote}${baseNotes}"\n`;
    }
    yaml += `\n`;
  });
